...
export default class App extends React.Component {
    state = {
        ...
        selectedSeconds: "3",
        duration: 5000,
        pattern: [0, 632, 632, 632, 632, 632, 632, 632, 632],
        underFive: false,
    };
    ...
    async _loadNewPlaybackInstance(playing) {
        ...
        const source = require('../assets/sounds/snoop.wav');
        const initialStatus = {
            shouldPlay: false,
            rate: 1.0,
    ...
    componentDidUpdate(prevProp, prevState) {
        if (this.state.remainingSeconds <= 6) {
            this.state.underFive = true;
        }
        ...
        return (
            <View style={styles.container}>
                ...
                {this.state.isRunning ? (
                    <Text style={this.state.underFive ? styles.timerText2 : styles.timerText}>{`${minutes}:${seconds}`}</Text>
                ) : (
                        this.renderPickers()
                    )}
                {this.state.isRunning ? (
